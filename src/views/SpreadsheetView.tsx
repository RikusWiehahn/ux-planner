...
export const SpreadsheetView = () => {
	...
	const renderStickyColumnHeaders = () => {
		return (
	...
	const renderEmptyState = () => {
		if (plan.planDoc.rootIds.length > 0) {
			return null;
		}
	...
	const renderGridCell = (placement: PlanNodeGridPlacement) => {
		...
		return (
			<div
				key={node.id}
				className={
					placement.rowStart === 1 ? "group border-r border-zinc-200 p-1.5" : "group border-r border-t border-zinc-200 p-1.5"
				}
				style={{
						...
						<div className="flex flex-wrap items-center gap-1 text-[11px] leading-none">
							...
							<div className="flex items-center gap-1">
								<div className="flex items-center gap-1 rounded-full border border-zinc-200 bg-white px-1.5 py-0 text-[10px] font-semibold">
									<span className={`${importanceValue ? "text-zinc-600" : "text-zinc-400"} font-mono`}>I</span>
									<TextInput
										...
										onChange={(nextValue) => {
											...
											const nextOrNull =
												next.importance === 0 && next.ease === 0 && next.timeHours === 0 ? null : next;
							...
							<div className="flex items-center gap-1">
								<div className="flex items-center gap-1 rounded-full border border-zinc-200 bg-white px-1.5 py-0 text-[10px] font-semibold">
									<span className={`${easeValue ? "text-zinc-600" : "text-zinc-400"} font-mono`}>E</span>
									<TextInput
										value={easeValue}
										onChange={(nextValue) => {
											const raw = nextValue.trim();
							...
							<div className="flex items-center gap-1">
								<div className="flex items-center gap-1 rounded-full border border-zinc-200 bg-white px-1.5 py-0 text-[10px] font-semibold">
									...
									/>
									<span className={`${timeHoursValue ? "text-zinc-600" : "text-zinc-400"} font-mono`}>hrs</span>
								</div>
							</div>
						</div>
					</div>
	...
	const renderGridBody = () => {
		return (
			...
			>
				{renderEmptyState()}
				{layout.placements.map((placement) => renderGridCell(placement))}
			</div>
		);
	};
